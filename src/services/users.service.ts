  ...
  static readonly updateUser = async (
    ...
    try {
      ...
      const fieldsToCheck: {
        ...
        { key: 'password', value: dataSend.password !== "" ? dataSend.password : undefined },
        ...
        { 
          ...
          compare: (a: number[], b: number[]) => JSON.stringify(a) !== JSON.stringify(b) 
        }
      ];
  
      // Verificar y actualizar los campos
      fieldsToCheck.forEach(({ key, value, compare }) => {
        if (value !== undefined && (compare ? compare(value, currentUser[key]) : value !== currentUser[key])) {
